...
namespace mygame
{
    ...
    public class LandModel
    {
        ...
        RoleModel[] roles = new RoleModel[6];           //陆地上角色
        ...
        public int GetEmptyNumber()                      //得到陆地上的空位置数
        {
        ...
        public Vector3 GetEmptyPosition()               //得到陆地上空位置坐标
        {
        ...
        public RoleModel DeleteRoleByName(string role_name)      //角色离开陆地
        {
            for (int i = 0; i < roles.Length; i++)
            {
    ...
    public class BoatModel
    {
        ...
        public void BoatMove()
        {
            if (boat_sign == -1)
            {
                move.MovePosition(new Vector3(30, -3.9F, -2.9F));
                boat_sign = 1;
            }
            else
            {
                move.MovePosition(new Vector3(-30, -4, -3));
                boat_sign = -1;
            }
        }//调用脚本，结合sign实现来回移动
